...
class Topology:
        def __init__(self,input_size):
                ...
                self.b[15] = self.getBiasVariable(self.output_size,'b_15')
        ...
        def getBiasVariable(self,shape_, name_):
                initial = tf.zeros([shape_],name=name_) 
...
class OperationMode:
        def __init__(self,train=False,new_init=False,validation=False,control=False,bias=False,zero_out_biases=False):
                self.train = train
                self.new_init = new_init
                self.validation = validation
                self.control = control
                self.bias = bias
                # special case for when you want to predict using the full graph
                # using no biases for the inner layers except the middle, which will be
                # controlled by a user
                self.zero_out_biases = zero_out_biases
...
class ANNeSynth:
        ...
        # generate one frame of frequencies, either using the middle layer, or the full input layer with or without middle_weight params
        def generate_audio(self, input_vals, bass_boost = False, full_mode = False, pitch_shift = 0, middle_weights = None) :
            outFreqs = None
            if full_mode :
                if middle_weights is not None :
                    self.reassign_middle_weights(middle_weights)
                outFreqs = np.roll(self._sess.run(self.outputLayer,feed_dict={self.x_:input_vals}), pitch_shift)
            else :
                outFreqs = np.roll(self._sess.run(self.outputLayer2,feed_dict={self.controller:input_vals}), pitch_shift)

            if bass_boost :
                bass_boost_arr = (np.exp(np.linspace(0.95,-0.95,self.topology.output_size)))

                outFreqs = outFreqs * bass_boost_arr
